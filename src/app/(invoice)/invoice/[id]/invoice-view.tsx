...
interface Item { desc: string; qty: string; unit: number; amount: number }
interface Section { title: string; items: Item[]; parts: number; labour: number }
interface InvoiceData {
  number: string; date: string; customer: string; make: string; model: string;
  vin: string; registration: string; mileage: string;
  sections: Section[]; totalParts: number; totalLabour: number;
  grandTotal: number; grandTotalWords: string;
}

const FALLBACK: InvoiceData = {
  number: "5171", date: "31/03/2026", make: "Mini", model: "F56",
  vin: "TM31955", registration: "110-B-AJM", mileage: "57,066 km", customer: "Mr Abu Mohd",
  sections: [
    {
    ...
    },
    {
      ...
      parts: 850, labour: 300,
    },
  ],
  totalParts: 3015, totalLabour: 450, grandTotal: 3465,
...
export default function InvoiceView({ id }: { id: string }) {
  ...
  useEffect(() => {
    const stored: InvoiceData[] = JSON.parse(localStorage.getItem("bn_invoices") || "[]");
    const found = stored.find((i) => i.number === id);
    setInvoice(found || (id === "5171" ? FALLBACK : null));
  }, [id]);

  if (!invoice) {
    return (
      <div className="min-h-screen bg-[#0A0A0A] flex items-center justify-center">
        <p className="text-white/40 text-sm">Invoice #{id} not found.</p>
      </div>
    );
  }
      ...
      >
        ...
        <div className="bg-[#0A0A0A] px-5 md:px-10 pt-6 pb-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <img src="/bnlogo__1_-removebg-preview.png" alt="BimmerNext"
            ...
            <div className="text-right shrink-0">
              <div className="inline-block border border-[#00C2C7] px-3 py-1 rounded mb-1.5">
                <span className="text-[#00C2C7] text-xs md:text-sm font-bold tracking-[0.2em] uppercase">Invoice</span>
              </div>
        ...
        <div className="bg-[#F5EFE6] px-5 md:px-10 py-4 md:py-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2 gap-x-6">
            <DR label="Customer" value={invoice.customer} highlight />
            <DR label="Vehicle" value={[invoice.make, invoice.model].filter(Boolean).join(" ") || "—"} highlight />
            <DR label="Date" value={invoice.date} />
            <DR label="VIN" value={invoice.vin || "—"} />
            <DR label="Order No." value={`#${invoice.number}`} />
            <DR label="Registration" value={invoice.registration || "—"} />
            <DR label="Mileage" value={invoice.mileage || "—"} />
          </div>
        </div>
            ...
            <div key={section.title}>
                ...
                <table className="w-full text-xs md:text-sm border-collapse min-w-[380px]">
                  <thead>
                    <tr className="bg-[#0A0A0A] text-white">
                      <th className="text-left px-3 md:px-4 py-2.5 font-semibold text-[10px] md:text-xs tracking-wider">Description</th>
                      <th className="text-center px-2 md:px-4 py-2.5 font-semibold text-[10px] md:text-xs tracking-wider w-10 md:w-14">Qty</th>
                      <th className="text-center px-2 md:px-4 py-2.5 font-semibold text-[10px] md:text-xs tracking-wider w-14 md:w-20">Unit</th>
                      <th className="text-right px-3 md:px-4 py-2.5 font-semibold text-[10px] md:text-xs tracking-wider w-20 md:w-28">Amt (AED)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {section.items.map((item, i) => (
                      <tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-[#F5EFE6]/50"}>
                        ...
                        <td className="px-2 md:px-4 py-2 text-center text-[#8A8A8A]">{(item.unit || 0).toLocaleString()}</td>
                        <td className="px-3 md:px-4 py-2 text-right font-semibold text-[#0A0A0A]">{(item.amount || 0).toLocaleString()}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-1 flex justify-end">
                <div className="min-w-[200px] md:min-w-[260px] border-t border-[#0A0A0A]/10">
                  <SR label="Parts" value={`${(section.parts || 0).toLocaleString()}/-`} />
                  <SR label="Labour" value={`${(section.labour || 0).toLocaleString()}/-`} />
                </div>
              </div>
            </div>
          ))}
...
function DR({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-baseline gap-2 text-xs md:text-sm">
      <span className="text-[#8A8A8A] w-24 shrink-0 text-[10px] md:text-xs font-semibold tracking-wider uppercase">{label}</span>
      <span className={`font-semibold break-all ${highlight ? "text-[#0A0A0A]" : "text-[#1A1A1A]"}`}>{value}</span>
    </div>
  );
}
